...
def train_single_run(X_train, Y_train, X_test, Y_test, input_dim, batch_size, step_size, hidden_units, max_epochs, momentum=0.8, weight_decay=1e-4):
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(num_batches):
            ...
            preds = (1.0 / (1.0 + np.exp(-logits)) >= 0.5).astype(int)
            batch_acc = np.mean((preds.flatten() == Y_batch.flatten()).astype(float))
        ...
        logging.info("Epoch %d: Test Loss=%.4f, Test Acc=%.4f", epoch+1, test_loss, test_acc)
...
def main():
    ...
    # (using fixed parameters: batch_size=128, step_size=0.001, hidden_units=100, max_epochs=50)
    ...
    # Produce output plot (using teacher's provided format)
